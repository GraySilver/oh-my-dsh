# @graysilver/oh-my-dsh-models

[English](README.md) | 中文

Oh My DSH 的模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和两个有序的首次使用弹窗：版本化产品欢迎页，以及按条件显示的 DeepSeek 官方连接步骤。连接步骤会先用尚未保存的密钥和端点调用 `llm.discoverModels`；测试通过前既不写设置，也不写凭据。测试被拒绝时，弹窗保持打开，密钥不会持久化。两个步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。

这个浏览器包所调用的带认证 DeepSeek discovery 由 `@graysilver/oh-my-dsh` Host 插件提供。它只把尚未保存的密钥发送到表单所示的 DeepSeek 端点，同时限制请求时长和响应字节数，只返回模型 id，并从错误中排除提供方响应正文。测试通过后才写入的先后顺序仍由浏览器包持有。

Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`、`settings.describe` 与 `credentials.describe`；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
欢迎步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。回环访问会通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确确认才会记录当前版本。非回环浏览器无法使用这项仅限 Host 的 namespace，因此确认仅在当前进程有效，重载后欢迎页会再次出现。

欢迎完成后，DeepSeek 步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染。只有没有可用提供方的用户才会被要求填写 DeepSeek 官方密钥。适配器已挂载且活跃、引用可写但尚未配置时，既有 `ProviderEditor` 会以仅凭据模式渲染。它先用表单中尚未保存的密钥和公共端点调用 `llm.discoverModels`；只有成功回复才会进入 `credentials.set`，传输或提供方错误都不会持久化任何内容。「稍后配置」只完成协调器当前这一轮。
