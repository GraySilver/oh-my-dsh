...
The Desktop build embeds the same WebUI in an Electron window and owns the local runtime process. Closing the window hides the app in the menu bar; choose Quit from the tray menu to stop the runtime. During development, run `pnpm oh-my-dsh:desktop`; package the `OhMyDSH` Universal `.app` and `.dmg` with `pnpm oh-my-dsh:desktop:package`. The package uses the official DeepSeek whale mark from `https://www.deepseek.com/favicon.ico` on an opaque white canvas, keeps the plugin dependency closure in the physical `Resources/app` tree, and boots the complete packaged Web profile before succeeding, then leaves artifacts in `apps/oh-my-dsh-desktop/release/`.
