...
The Desktop build embeds the same WebUI in an Electron window and owns the local runtime process. Closing the window hides the app in the menu bar; choose Quit from the tray menu to stop the runtime. During development, run `pnpm oh-my-dsh:desktop`; package the `OhMyDSH` Universal `.app` and `.dmg` with `pnpm oh-my-dsh:desktop:package`. The package uses the DeepSeek whale icon from LobeHub's [light static PNG set](https://raw.githubusercontent.com/lobehub/lobe-icons/refs/heads/master/packages/static-png/light/deepseek.png) on a white rounded-square canvas with transparent outer corners for the standard macOS icon mask, keeps the plugin dependency closure in the physical `Resources/app` tree, and boots the complete packaged Web profile before succeeding, then leaves artifacts in `apps/oh-my-dsh-desktop/release/`.

Desktop always binds its local runtime to `0.0.0.0:3080` so another trusted device can use the LAN URL shown in the tray menu. LAN clients have the same permissions as localhost, including settings, credentials, filesystem access, and approved commands. Use this mode only on a trusted network. The Desktop build refuses to start when port 3080 is already occupied instead of replacing another process.
