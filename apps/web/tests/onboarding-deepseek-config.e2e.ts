...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    const credentialStep = page.getByRole('dialog', { name: '连接 DeepSeek' })
    ...
    await credentialStep.getByRole('button', { name: '测试连接、保存并继续' }).click()
    ...
    expect(await page.getByRole('dialog', { name: '连接 DeepSeek' }).count()).toBe(0)
    ...
    await page.addInitScript(() => {
      ...
      setInterval(() => {
        if (document.querySelector(
          '[role="dialog"][aria-label="欢迎使用 Oh My DSH"], '
          + '[role="dialog"][aria-label="连接 DeepSeek"]',
        ) !== null) {
