...
Desktop 版本会在 Electron 窗口内承载同一套 WebUI，并统一管理本地运行时进程。关闭窗口只会把应用隐藏到菜单栏；从 Tray 菜单选择退出才会停止运行时。开发时运行 `pnpm oh-my-dsh:desktop`，使用 `pnpm oh-my-dsh:desktop:package` 构建支持 arm64 和 x64 的 Universal `.app` 与 `.dmg`。打包产物会把完整插件依赖闭包放在物理 `Resources/app` 目录树中，并在命令成功前启动完整的 Web profile，产物写入 `apps/oh-my-dsh-desktop/release/`。
